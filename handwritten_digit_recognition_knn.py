# -*- coding: utf-8 -*-
"""handwritten-digit-recognition-knn.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gpJ9hB7P97l8_LK4rytA4yu0UhMENHcH
...
k_value = 3  # Here i am starting with 3 as my inital value,but i am going to experiment with diffrent values to see how it effects my model
...
# Here we are making use of formatted string literals to print the output
print(f"Accuracy: {accuracy:.4f}")
print(f"Precision: {precision:.4f}")
print(f"Recall: {recall:.4f}")
print(f"F1 Score: {f1_score:.4f}")

# Make predictions on new handwritten digits (you can use X_test or new data)
new_digit_predictions = knn_classifier.predict(x_test[:5])
# Here we are going to make predictions on first 5 images from the trainig dataset
# Visualize the predictions
# Here A loop iterates over the first five images in the testing set.
# Each image is displayed as a subplot using plt.imshow.
# The title of each subplot includes the predicted digit label.
# plt.axis('off') removes the axis labels for better visualization.
# Finally, plt.show() displays the entire set of subplots.
for i in range(5):
    plt.subplot(1, 5, i + 1)
